...
class NasdaqProvider(SymbolProvider):
    """
    Fetches the live Nasdaq-100 constituent list from Wikipedia.
    Caches result for `cache_days` days.
    Dots in tickers are converted to dashes (e.g. BRK.B -> BRK-B) for Yahoo Finance.
    """

    WIKIPEDIA_URL  = "https://en.wikipedia.org/wiki/Nasdaq-100"
    CACHE_FILENAME = "nasdaq100_symbols.json"
    ...
    def _load_cache(self) -> list[str] | None:
        ...
        if symbols:
            log.info(f"NasdaqProvider: loaded {len(symbols)} symbols from cache ({age.days}d old)")
        return symbols or None

    def _fetch_and_cache(self) -> list[str]:
        log.info("NasdaqProvider: fetching live Nasdaq-100 list from Wikipedia...")
        try:
            import requests
            ...
            # The Nasdaq-100 Wikipedia page table has id="constituents"
            ...
            if not symbols:
                raise ValueError("Parsed 0 symbols from Nasdaq-100 Wikipedia table")

            log.info(f"NasdaqProvider: fetched {len(symbols)} symbols")
            ...
            log.error(f"NasdaqProvider: Wikipedia fetch failed -- {exc}")
            if os.path.exists(self.cache_path):
                log.warning("NasdaqProvider: using stale cache as fallback")
                with open(self.cache_path) as f:
                    return json.load(f).get("symbols", [])
            raise RuntimeError("No Nasdaq-100 symbols available and no cache to fall back on.") from exc
